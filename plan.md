...
- [ ] 新建 `red-packet` 模块。
- [ ] 表达金额字段使用 Prisma `Decimal`。
- [ ] 发红包事务中扣余额、创建红包、写余额流水、初始化 Redis 预拆金额。
- [ ] 普通红包和随机红包拆分逻辑从 Java 迁移并补充边界校验。
- [ ] 领取红包使用 Redis Lua 原子判断重复领取和弹出金额。
- [ ] PostgreSQL 更新失败时补偿 Redis。
- [ ] 领取成功后增加余额、写领取记录、写余额流水、更新红包剩余金额和状态。
- [ ] 查询红包详情返回领取列表、发送人、封面文案、总额、数量、剩余、状态。
- [ ] 实现过期扫描和退款。
...
- [ ] 发布朋友圈保存文本和媒体 URL。
- [ ] 删除朋友圈使用软删除语义。
- [ ] 点赞、取消点赞使用软删除或状态字段，避免重复点赞。
- [ ] 评论支持父评论。
- [ ] 删除评论时标记子评论删除。
- [ ] 查询朋友圈时只返回自己和好友可见范围。
- [ ] 增量列表返回新增和删除集合。
- [ ] 发布、点赞、评论通过 realtime 推送通知。
